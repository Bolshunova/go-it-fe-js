...
class Hamburger {
    constructor(size, stuffing) {
      ...
      removeTopping(topping) {
        if( this._toppings.length === 0 ) return;
      ...
      calculatePrice() {
        const priceSizeBurger = Hamburger.SIZES[this._size].price;
        const priceStuffingsBurger = Hamburger.STUFFINGS[this._stuffing].price;
        const priseToppingsBurger = this._toppings.reduce((acc, elem) => acc + Hamburger.TOPPINGS[elem].price, 0);
        
        const totalPriceBurger = priceSizeBurger + priceStuffingsBurger + priseToppingsBurger;
        return totalPriceBurger;
      }
  
      calculateCalories() {
        const caloriesSizeBurger = Hamburger.SIZES[this._size].calories;
        const caloriesStuffingsBurger = Hamburger.STUFFINGS[this._stuffing].calories;
        const caloriesToppingsBurger = this._toppings.reduce((acc, elem) => acc + Hamburger.TOPPINGS[elem].calories, 0);
        
        const totalCaloriesBurger = caloriesSizeBurger + caloriesStuffingsBurger + caloriesToppingsBurger;
        return totalCaloriesBurger;
      }
    }
        Hamburger.SIZE_SMALL = 'SIZE_SMALL';
  ...
  // // Сколько стоит?
  console.log("Price: with 1 sauce", hamburger.calculatePrice());

  // // Спросим сколько там калорий
  console.log("Calories: with 1 sauce", hamburger.calculateCalories());
  
  // // Я тут передумал и решил добавить еще соус
  hamburger.addTopping(Hamburger.TOPPING_SAUCE);
  
  // // Смотрим сколько добавок
  console.log("Hamburger has %d toppings", hamburger.toppings.length); // 2

  // // А сколько теперь стоит?
  console.log("Price with 2 sauce: ", hamburger.calculatePrice());

  // // Спросим сколько там сейчас калорий
  console.log("Calories with 2 sauce: ", hamburger.calculateCalories());
 ...
 // // А сколько теперь стоит?
 console.log("Price with 1 sauce (after delete): ", hamburger.calculatePrice());

 // // Спросим сколько там калорий
 console.log("Calories: with 1 sauce (after delete):", hamburger.calculateCalories());
